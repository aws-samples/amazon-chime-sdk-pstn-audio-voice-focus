...
def lambda_handler(event, context):
    ...
    if event_type == "NEW_INBOUND_CALL":
        return new_call_handler(call_id, event)
...
def new_call_handler(call_id, event):
    if dial_number:
        ...
        return response(
            ...
            play_and_get_digits_action(
                call_id, "^(?!1900)1[0-9][0-9][0-9](\d{7})$", "welcome_vf_demo.wav", "invalid_entry.wav"
            ),
        )
...
def enable_voicefocus(event):
    ...
    for call in event["CallDetails"]["Participants"]:
        logger.info("CALL {}".format(call["CallId"]))
...
def control_voicefocus(call_id, event):
    ...
    for call in event["CallDetails"]["Participants"]:
        logger.info("CALL {}".format(call["CallId"]))
        actions.append(voicefocus_action(call["CallId"], enabled))
    return response(actions[0], actions[1])
...
def play_and_get_digits_action(call_id, regex, audio_file, failure_audio_file):
    return {
        ...
        "Parameters": {
            ...
            "FailureAudioSource": {"Type": "S3", "BucketName": wav_bucket, "Key": failure_audio_file},
...
def pause_action(call_id):
    return {"Type": "Pause", "Parameters": {"CallId": call_id, "DurationInMilliseconds": "3000"}}
